...
class Crawler {
  ...
  async _setLocalStorage() {
    if (!this._options.localStorage || isEmpty(this._options.localStorage)) return;

    const localStorage = this._options.localStorage;
    await this._page.evaluate((localStorage) => { window.localStorage = localStorage })
  }
   ...
   * @private
   */
  async _setSessionStorage() {
    if (!this._options.sessionStorage || isEmpty(this._options.sessionStorage)) return;

    const sessionStorage = this._options.sessionStorage;
    await this._page.evaluate((sessionStorage) => { window.sessionStorage = sessionStorage })
  }
